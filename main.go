...
func init() {
	...
	str := pterm.DefaultHeader.WithBackgroundStyle(pterm.NewStyle(pterm.BgRed)).WithMargin(10).Sprintf(
		"FBOSINT - Facebook Account OSINT")
...
func main() {
	...
	if r.User != nil {
		pterm.Printfln("[%s] Facebook Name : %s", pterm.Green("+"), pterm.Green(r.User.Name))
		pterm.Printfln("[%s] Facebook Profile saved : %s", pterm.Green("+"), pterm.Green(r.User.ImagePath))
